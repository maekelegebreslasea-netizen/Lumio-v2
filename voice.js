var _vc={active:false,recording:false,thinking:false,speaking:false,stream:null,mediaRecorder:null,audioChunks:[],history:[],checkInterval:null,lastSoundTime:0,hasSound:false,SILENCE_MS:2000,VOL_THRESHOLD:6,systemPrompt:"You are Lumio, a friendly AI study tutor in a voice call. Keep answers SHORT — 2-4 sentences. Be warm and natural."};
...
async function vcStart(){
  ...
  try{
    ...
    var greeting="Hi"+(uname?", "+uname:"")+"! I'm Lumio. "+(subj?"Ready to study "+subj+" together? ":"")+"What would you like to learn today?";
    _vc.history.push({role:"assistant",content:greeting});
    vcTxt(greeting);vcSt("Lumio is speaking...");vcMic("thinking");
...
async function vcTranscribe(analyser,buf){
  ...
  try{
    ...
    var d=await resp.json();var txt=d.text?d.text.trim():"";
    if(!txt||txt.length<2){vcResume(analyser,buf);return;}
    vcTxt("You: "+txt);vcSt("Lumio is thinking...");
    ...
    var d2=await r2.json();var reply=d2.text||"Could you repeat that?";
  ...
  }catch(e){vcSt("Error — try again");vcResume(analyser,buf);}
}
function vcSpeak(text,onDone){
  if(!window.speechSynthesis){if(onDone)onDone();return;}
  speechSynthesis.cancel();_vc.speaking=true;vcMic("thinking");vcSt("Lumio is speaking...");document.getElementById("vcWave").style.display="flex";
