...
function llGo(i){
  ...
  // Dispatch custom event — React listens and updates state
  var tabs=["subjects","lesson","dual","games","profile"];
  document.dispatchEvent(new CustomEvent("lx-nav",{detail:tabs[i]}));
}
function llNew(){llGo(0);setTimeout(function(){document.dispatchEvent(new CustomEvent("lx-newsub"));},300);}
    ...
    if(list.children.length!==cards.length+0){
      ...
      cards.forEach(function(card,i){
        ...
        btn.onclick=function(){
          // Dispatch event to open subject
          var nid2=card.querySelector("[data-nid]");
          var sid=nid2?nid2.dataset.nid:"";
          if(sid){
            document.dispatchEvent(new CustomEvent("lx-opensubj",{detail:sid}));
          } else {
            card.click();
          }
