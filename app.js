function App(){
  ...
  // Listen for desktop sidebar navigation events
  useEffect(()=>{
    function onNav(e){setTab(e.detail)}
    function onNewSub(){setShowNewSub(true)}
    function onOpenSubj(e){
      var s=subjects.find(function(s){return s.id===e.detail});
      if(s){setActive(s);setTab("subject");}
    }
    document.addEventListener("lx-nav",onNav);
    document.addEventListener("lx-newsub",onNewSub);
    document.addEventListener("lx-opensubj",onOpenSubj);
    // Store subjects for desktop
    window._lxSubjects=subjects;
    return function(){
      document.removeEventListener("lx-nav",onNav);
      document.removeEventListener("lx-newsub",onNewSub);
      document.removeEventListener("lx-opensubj",onOpenSubj);
    };
  },[subjects,setTab,setActive,setShowNewSub]);
