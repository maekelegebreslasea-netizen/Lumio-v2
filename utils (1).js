...
const db={
  ...
  saveSub:async(s,uid)=>{
    const r=await getSupa().from("subjects").upsert({id:s.id,user_id:uid,name:s.name,emoji:s.emoji,color:s.color,sessions:s.sessions||0});
    if(r.error)console.error("[DB] saveSub error:",r.error);
    return r;
  },
  ...
  saveMat:async(m,sid,uid)=>{
    const r=await getSupa().from("materials").upsert({id:m.id,subject_id:sid,user_id:uid,name:m.name,kind:m.kind,text_content:m.text||""});
    if(r.error)console.error("[DB] saveMat error:",r.error);
    return r;
  },
  delMat:async id=>getSupa().from("materials").delete().eq("id",id),
  saveSession:async(key,uid,msgs)=>{
    try{
      const r=await getSupa().from("sessions").upsert(
        {subject_id:key,user_id:uid,messages:JSON.stringify(msgs.slice(-40)),updated_at:new Date().toISOString()},
        {onConflict:"subject_id,user_id"}
      );
      if(r.error)console.error("[DB] saveSession error:",r.error);
    }catch(e){console.error("[DB] saveSession failed:",e);}
  },
...
const sysLuxori=(s,m,l,lang)=>`You are Luxori, expert AI tutor for "${s}". Respond ONLY in ${lang}. Base 100% on provided materials. Use **bold** for key terms. ${lvMap[l]||lvMap.medium} MAX 3 sentences + 1 question.\nMATERIALS:\n${matText(m)||"None."}`;
...
const sysChat=(s,m,lang)=>`You are Luxori AI for "${s}". Respond in ${lang}. Base 100% on materials. **Bold** key terms. Detailed exam-ready answers. Cite [Source: "name"].\nMATERIALS:\n${matText(m)||"None."}`;
...
function MsgList({msgs,loading,loadingRole,onSend,showHints,langCode,speechRate=1}){
  ...
  const rl=r=>r==="atlas"?"ATLAS":r==="spark"?"SPARK":"LUXORI";
